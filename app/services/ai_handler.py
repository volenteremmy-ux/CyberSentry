...
class AIHandler:
    ...
    def predict_intent(self, text: str, sender: str = "Unknown"):
        """
        Tries HuggingFace -> Fails? -> Tries Gemini -> Fails? -> Returns 0 (Hardcoded Fallback)
        """
        ...
        try:
            ...
            # TIMEOUT IS CRITICAL: If it takes > 2s, we abort immediately
            response = requests.post(
                settings.HF_MODEL_URL, 
                headers=self.hf_headers, 
                json=payload, 
            ...
            if response.status_code == 200:
                ...
                # If 404/500, print error but don't crash. Just move to Gemini.
                print(f"   ⚠️ HF Error {response.status_code}. Switching...")

        except Exception as e:
            print(f"   ⚠️ HF Skipped ({str(e)[:20]}...). Switching...")
        ...
        try:
            prompt = f"""
            Act as a security system. Analyze this SMS.
            Sender: "{sender}"
            Message: "{text}"
